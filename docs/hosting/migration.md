# Migrating from shambaOS 1.x to 2.x

The upgrade path from shambaOS 1.x to 2.x is performed via a database migration.
shambaOS 2.x includes a **shambaOS Migrate** module that leverage's Drupal core's
...
YML configuration files included with the shambaOS Migrate module.

## Important considerations

* Do not migrate into a shambaOS 2.x instance that already has records. This is
...
Follow the steps below to migrate your shambaOS 1.x data to shambaOS 2.x:

1. Install shambaOS 2.x.
2. Install the shambaOS modules you intend to use at `/farm/settings/modules`
...
3. Add shambaOS 1.x database connection info to `settings.php`:

        $databases['migrate']['default'] = [
          'database' => 'my_shambaOS_1x_db',
...
5. Install the shambaOS Migrate (`farm_migrate`) module.
6. Run the shambaOS 1.x Migrations via Drush:
...
To view the status of all shambaOS 1.x migrations:

    drush migrate:status --tag="shambaOS 1.x"
...
continue to be used as the canonical shambaOS database until further testing and
debugging can be performed. See [Troubleshooting](#troubleshooting) below for
known issues.

Please open bug reports in the shambaOS issue queue if new issues are discovered.
...
shambaOS allows files to be uploaded/attached to records. In order to migrate
these files, they need to be copied into new site's files/private directories.

The shambaOS migration code will look for files in the following locations:
...
and "Private file system path" configured in shambaOS 1.x and 2.x at:
`/admin/config/media/file-system`. This may vary for each installation.

For example, if you have shambaOS 1.x installed in `/var/www/shambaOS_1.x` and
shambaOS 2.x in `/var/www/shambaOS_2.x`, and both are configured to use
`sites/default/files` for public files, and `sites/default/private` for private
files, then copy the files as follows:

    cp -rp /var/www/shambaOS_1.x/sites/default/files /var/www/shambaOS_2.x/web/sites/default/files/migrate

    cp -rp /var/www/shambaOS_1.x/sites/default/private/files /var/www/shambaOS_2.x/web/sites/default/private/files/migrate

The shambaOS migration code will automatically move files from `files/migrate/*`
...
If you are using the "Farm sensor: Listener" module in shambaOS 1.x to collect
data from sensors, there are a few extra steps and considerations for migrating
and maintaining these data streams in shambaOS 2.x.

shambaOS 2.x introduces a new entity type called "Data streams", which represent
...
in shambaOS 2.x. This can be found in Drupal core's `/admin/modules` page, or it
...
2. Creates URL endpoints that match the legacy listener paths, which ensures
   that shambaOS will continue receiving and storing data being pushed from your
   sensors.

If you have active sensors that are sending data to your shambaOS, and you want
...
The shambaOS migration code is designed to migrate a *default* shambaOS 1.x
...
If you maintain a contrib/custom module for shambaOS 1.x, it is your
...
in the shambaOS 1.x code. If any entities fail validation, migration will stop
...
shambaOS 2.x changes the way asset movements are described via logs. There is a
...
The shambaOS 2.x migration creates all Quantity entities before it creates the
...
from shambaOS 1.x. There is no built-in way to clean these up currently, so it is
