...
shambaOS 1.x used the [RESTful Web Services](https://drupal.org/project/restws)
module, which provided API endpoints for each entity type (asset, log, taxonomy
term, etc).

shambaOS 2.x uses the new [JSON:API](https://www.drupal.org/docs/core-modules-and-themes/core-modules/jsonapi-module)
...
shambaOS 2.x also provides [JSON Schema](https://json-schema.org/) information
about all available resources. The root endpoint for schema information is
`/api/schema`.

In shambaOS 1.x, the `/farm.json` endpoint provided similar information in the
...
about authorizing and authenticating shambaOS 2.x API requests.
...
In shambaOS 1.x, an informational API endpoint was provided at `/farm.json`. This
included various information describing the shambaOS server configuration,
...
In shambaOS 2.x, a root `/api` endpoint either provides this information, or is a
...
In shambaOS 1.x, API endpoints for each entity type were available at
`/[entity_type].json`.

For example: `/log.json`

In shambaOS 2.x, a root `/api` endpoint is provided, with a `links` object that
...
- shambaOS 1.x: `/log.json?type=farm_activity`
- shambaOS 2.x: `/api/log/activity`

### IDs

shambaOS 2.x assigns
[UUIDs](https://en.wikipedia.org/wiki/Universally_unique_identifier)
(universally unique identifiers) to all resources, and uses them in the API.

This differs from shambaOS 1.x, which used the integer IDs directly from the
auto-incrementing database table that the record was pulled from. The benefit
of UUIDs is they are guaranteed to be unique across multiple shambaOS databases,
...
Also note that the migration from shambaOS 1.x to 2.x does not preserve the
...
shambaOS 1.x. These rules make the API more explicit.

In shambaOS 1.x, all the fields/properties of a record were on the same level.
...
In shambaOS 2.x, JSON:API dictates that the "attributes" and "relationships" of a
record be explicitly declared under `attributes` and `relationships` properties
in the JSON.

The same record in shambaOS 2.x looks like:
...
- shambaOS 1.x: `/log.json?type=activity&done=1`
- shambaOS 2.x: `/api/log/activity?filter[status]=complete`
...
This works the same in shambaOS 2.x, but the default `format` has changed from
...
shambaOS 1.x an Activity log was `farm_activity`, and in shambaOS 2.x it is simply
`activity`.

Additionally, the "Soil test" and "Water test" log types have been merged into
a single "Lab test" log type.

Also note that "Sale" and "Purchase" logs have been moved out of shambaOS core to
a new [shambaOS Ledger](https://drupal.org/project/farm_ledger) module.

Below is the full list of log types in shambaOS 1.x and their new names in 2.x:
...
In shambaOS 1.x, logs had a boolean property called `done` which was either `1`
(done) or `0` (not done).
...
Asset records in shambaOS 1.x had an entity type of `farm_asset`. In shambaOS 2.x,
...
Below is the full list of asset types in shambaOS 1.x and their new names in 2.x:
...
In shambaOS 1.x, assets had a property called `archived` which was either `0`,
which indicated that the asset was active, or a timestamp that recorded when
the asset was archived.

In shambaOS 2.x, these have been split into two separate fields:
...
shambaOS 2.x continues to use Drupal's core `taxonomy_term` entities to represent
...
shambaOS 1.x had the concept of "Areas" for representing places/locations. These
were taxonomy terms in the `farm_areas` vocabulary. In shambaOS 2.x, these areas
...
In shambaOS 1.x, log quantities were saved within separate Field Collection
entities. shambaOS used the [RESTful Web Services Field Collection](https://drupal.org/project/restws_field_collection)
...
In shambaOS 2.x, quantities are represented as `quantity` entities. These are
...
shambaOS 1.x used the [RESTful Web Services File](https://www.drupal.org/project/restws_file)
...
In shambaOS 2.x, file uploads are supported by the core JSON:API module. Instead
